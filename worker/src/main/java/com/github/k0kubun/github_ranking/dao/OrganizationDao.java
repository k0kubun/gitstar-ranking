...
{
    @SqlQuery("select id, type, stargazers_count from users where type = 'Organization' order by stargazers_count desc, id desc limit :limit")
    @Mapper(OrganizationMapper.class)
    List<Organization> starsDescFirstOrgs(@Bind("limit") Integer limit);

    @SqlQuery("select id, type, stargazers_count from users where type = 'Organization' and " +
    ...
    class OrganizationMapper implements ResultSetMapper<Organization>
    {
        @Override
        public Organization map(int index, ResultSet r, StatementContext ctx) throws SQLException
        {
            return new Organization(r.getInt("id"), r.getInt("stargazers_count"));
        }
    }
}
